#!/bin/bash
# ============================================================================
# Single SLURM job: ResNet-18 assistant (KD from ResNet-50) -> MobileNet
# distilled from the assistant. Train + eval for both, 4409-style layout.
#
# Layout:
#   experiments/logs/slurm-train-eval-<JOBID>/
#     assistant_r18/train, assistant_r18/eval
#     mobilenet_from_r18/train, mobilenet_from_r18/eval
#     pipeline_summary.txt
# ============================================================================

#SBATCH --job-name=kd-4409-assistant
...
# --- Phase 1: Assistant ResNet-18 distilled from ResNet-50 ---
teacher_r50_ckpt="/home/brbdnl01e03e017o/dl26-projects/experiments/checkpoints/teacher_finetune_best.pth"
assistant_cfg="experiments/configs/distillation_at_4409_assistant_r18.yaml"
assistant_train_dir="$ROOT_DIR/assistant_r18/train"
assistant_eval_dir="$ROOT_DIR/assistant_r18/eval"
assistant_ckpt_dir="$CHECKPOINT_ROOT/resnet18"
assistant_ckpt="$assistant_ckpt_dir/distillation_best.pth"

run_training "assistant_r18" "$assistant_cfg" "$assistant_train_dir" \
    "training.checkpoint_dir=$assistant_ckpt_dir distillation.teacher_checkpoint=$teacher_r50_ckpt logging.run_name=assistant_r18_kd_4409"

if [ -f "$assistant_ckpt" ]; then
    run_evaluation "assistant_r18" "$assistant_cfg" "$assistant_ckpt" "$assistant_eval_dir"
else
    overall_status="FAILED"
...
append_summary "assistant_r18" "$assistant_train_dir" "$assistant_eval_dir" "$assistant_ckpt"

# --- Phase 2: MobileNet distilled from ResNet-18 assistant ---
mobilenet_cfg="experiments/configs/distillation_at_4409_mobilenet_from_r18.yaml"
mobilenet_train_dir="$ROOT_DIR/mobilenet_from_r18/train"
mobilenet_eval_dir="$ROOT_DIR/mobilenet_from_r18/eval"
mobilenet_ckpt_dir="$CHECKPOINT_ROOT/mobilenet"
mobilenet_ckpt="$mobilenet_ckpt_dir/distillation_best.pth"

run_training "mobilenet_from_r18" "$mobilenet_cfg" "$mobilenet_train_dir" \
    "training.checkpoint_dir=$mobilenet_ckpt_dir distillation.teacher_checkpoint=$assistant_ckpt logging.run_name=mobilenet_from_r18_kd_4409"

if [ -f "$mobilenet_ckpt" ]; then
    run_evaluation "mobilenet_from_r18" "$mobilenet_cfg" "$mobilenet_ckpt" "$mobilenet_eval_dir"
else
    overall_status="FAILED"
...
append_summary "mobilenet_from_r18" "$mobilenet_train_dir" "$mobilenet_eval_dir" "$mobilenet_ckpt"
