#!/bin/bash
# =============================================================================
# submit_pure_kd_t10.sh  —  Run Pure Knowledge Distillation (T=10, Alpha=0.9)
#
# Runs:
#   1. KD Pure (T=10, Alpha=0.9, no AT)  → train + eval
#
# Usage:
#   export TEACHER_CKPT="/home/.../teacher_finetune_best.pth"
#   sbatch cluster/submit_pure_kd_t10.sh
# =============================================================================

#SBATCH --job-name=kd-pure-t10
...
#SBATCH --output=logs/slurm-pure-kd-%j.log
...
# Directory layout
# ---------------------------------------------------------------------------
ROOT_DIR="$PROJECT_DIR/experiments/logs/slurm-pure-kd-${SLURM_JOB_ID:-local}"
CHECKPOINT_ROOT="$PROJECT_DIR/experiments/checkpoints/slurm-pure-kd-${SLURM_JOB_ID:-local}"
...
# ---------------------------------------------------------------------------
{
    echo "Pipeline Summary — Pure KD (T=10, Alpha=0.9)"
    echo "============================================================"
    echo "job_tag: slurm-pure-kd-${SLURM_JOB_ID:-local}"
...
# 1) KD Pure
# ---------------------------------------------------------------------------
pure_kd_cfg="experiments/configs/distillation_t10_a09_pure.yaml"
pure_kd_train_dir="$ROOT_DIR/pure_kd/train"
pure_kd_eval_dir="$ROOT_DIR/pure_kd/eval"
pure_kd_ckpt_dir="$CHECKPOINT_ROOT/pure_kd"
pure_kd_ckpt="$pure_kd_ckpt_dir/distillation_best.pth"

run_training "pure_kd" "$pure_kd_cfg" "$pure_kd_train_dir" \
    "training.checkpoint_dir=$pure_kd_ckpt_dir distillation.teacher_checkpoint=$TEACHER_CKPT logging.run_name=kd_pure_t10"

if [ -f "$pure_kd_ckpt" ]; then
    run_evaluation "pure_kd" "$pure_kd_cfg" "$pure_kd_ckpt" "$pure_kd_eval_dir"
else
    overall_status="FAILED"
    mkdir -p "$pure_kd_eval_dir"
    touch "$pure_kd_eval_dir/status_FAILED"
fi
append_summary "pure_kd" "$pure_kd_train_dir" "$pure_kd_eval_dir" "$pure_kd_ckpt"
...
echo ""
echo "============================================================"
echo "Pure KD experiment completed."
