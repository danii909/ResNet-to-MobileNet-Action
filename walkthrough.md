# Cross-Frame Knowledge Distillation — Implementation Walkthrough

## Obiettivo

Implementare la **Cross-Frame KD** dove il Teacher (ResNet-50) riceve 24 frame e lo Student (MobileNet3D) riceve 16 frame sottocampionati dalla stessa clip. Lo Student impara a mimare le predizioni a 24-frame usando solo 16 frame, massimizzando il throughput in inferenza.

## File Modificati / Creati

### [NEW] [temporal.py](file:///I:/Development%202.0/KD_Project/src/training/temporal.py)

Funzione `temporal_subsample(clips, num_frames)` — il cuore del sottocampionamento temporale:

- **Input**: `[B, C, T, H, W]` con `T = teacher_frames` (es. 24)
- **Output**: `[B, C, num_frames, H, W]` (es. 16)
- **Strategia**: `torch.linspace(0, T-1, N).long()` → indici uniformemente spaziati
- **Efficienza**: opera su GPU via `index_select(dim=2)`, zero overhead CPU↔GPU
- **Safety**: validazione 5D, fast-path se `num_frames == T`

Per 24→16, gli indici selezionati sono:
```
[0, 1, 3, 4, 6, 7, 9, 10, 12, 13, 15, 16, 18, 19, 21, 23]
```
Copertura uniforme dell'intero span temporale.

---

### [MODIFY] [trainer.py](file:///I:/Development%202.0/KD_Project/src/training/trainer.py)
...
| Area | Modifica |
|---|---|
| `__init__` | Legge `distillation.student_frames` dal config. Se presente e < `dataset.num_frames`, attiva cross-frame KD |
| `_get_student_clips()` | Nuovo metodo helper che applica `temporal_subsample()` se cross-frame è attivo |
| `_compute_loss()` | Il Teacher riceve `clips` (24f), lo Student riceve `_get_student_clips(clips)` (16f). `torch.no_grad()` sul Teacher |
| `_evaluate()` | Lo Student è valutato **esclusivamente** sui suoi 16 frame — accuratezza reale di inference |
| `_write_config_summary()` | Logga `student_frames` nel summary se cross-frame è attivo |

> [!IMPORTANT]
> **Backward compatibility**: Se `distillation.student_frames` non è presente nel config, `self.student_frames = None` e il comportamento è identico alla KD standard (stesso input per Teacher e Student). Nessun esperimento esistente viene impattato.

---

### [NEW] [crossframe_kd_t24_s16.yaml](file:///I:/Development%202.0/KD_Project/experiments/configs/valid/crossframe_kd_t24_s16.yaml)

Config di esempio pronto per l'uso. La chiave cruciale:

```yaml
dataset:
  num_frames: 24          # Il DataLoader produce clip a 24 frame

distillation:
  student_frames: 16      # Sub-sampling a 16 frame per lo Student
...
Esempio di lancio sul cluster:

```bash
python -m src.training.train \
    --config experiments/configs/valid/crossframe_kd_t24_s16.yaml \
    --override distillation.teacher_checkpoint=<path_al_tuo_checkpoint>
```

Oppure nel SLURM script, sovrascrivendo da linea di comando:
```bash
--override distillation.student_frames=16 dataset.num_frames=24
```
...
```mermaid
flowchart LR
    DL["DataLoader<br/>[B, 3, 24, H, W]"] --> T["Teacher (ResNet-50)<br/>torch.no_grad()"]
    DL --> SUB["temporal_subsample<br/>24f → 16f"]
    SUB --> S["Student (MobileNet3D)<br/>grad enabled"]
    T --> TL["teacher_logits"]
    S --> SL["student_logits"]
    TL --> LOSS["KDLoss<br/>KL + CE"]
    SL --> LOSS
    LOSS --> BP["backward()"]
```
...
- ✅ `temporal.py` — syntax check OK
- ✅ `trainer.py` — syntax check OK  
- ✅ Backward compatibility — `student_frames=None` disattiva cross-frame
- ✅ Config di esempio creato
- ⬜ Test end-to-end sul cluster (richiede PyTorch + GPU)
